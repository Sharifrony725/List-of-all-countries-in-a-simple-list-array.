...
function printCountryOptions() {
  ...
  if (countryList.hasOwnProperty(key)) {
    // JSON format
    json.value += `{"name": "${countryList[key]}", "code": "${key}"},\n`;
    
    // HTML Format (indented two spaces)
    html.value += `  <option value="${key}">${countryList[key]}</option>\n`;
    
    // HTML Code Format (indented two spaces)
    htmlcode.value += `  &lt;option value="${key}"&gt;${countryList[key]}&lt;/option&gt;\n`;
  }
}
}
